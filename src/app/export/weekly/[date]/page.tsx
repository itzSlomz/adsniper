...
import { mentionsMaxItemsPerCall } from "@/lib/mentions/config";
import { exportFooter } from "@/lib/mentions/copy";
...
export default async function WeeklyExportPage(
  ...
  // The narrative above carries a conversation paragraph only when the
  // brief's facts did (§9.1); then, and only then, the footer discloses
  // what that sample is and is not (§8.8).
  const facts = briefing?.factsJson;
  const briefHasMentions =
    facts != null && typeof facts === "object" && !Array.isArray(facts) && (facts as { mentions?: unknown }).mentions != null;
  const conversationFooter = briefHasMentions ? exportFooter(mentionsMaxItemsPerCall()) : null;
  ...
  return (
    ...
    >
      ...
      <footer style={{ marginTop: 12, paddingTop: 6, borderTop: "1px solid #d5d2d2", fontSize: 7, color: "#8a8a8a", lineHeight: 1.5 }}>
        ...
        {conversationFooter && (
          <>
            <div style={{ marginTop: 3 }}>{conversationFooter.en}</div>
            <div dir="rtl" style={{ textAlign: "right" }}>{conversationFooter.ar}</div>
          </>
        )}
        <div style={{ marginTop: 3 }}>Generated by MarketingSpy.</div>
      </footer>
    </main>
  );
}
